...
init([ Addr, Port, Ttl ]) ->
    ...
    State = #state{ recvsock = RecvSocket,
...
discover(State) ->
    ...
    Message = [ "DISCOVER ", Mac, " ", <<Time:64>>, " ", NodeString ],
    gen_udp:send(State#state.sendsock,
                 State#state.addr,
                 State#state.port,
                 Message).
...
process_packet ("DISCOVER " ++ Rest, IP, InPortNo, State) -> 
    ...
    try
        <<Mac:20/binary, " ", Time:64, " ", NodeString/binary>> = list_to_binary(Rest),
        ...
        case { mac([ <<Time:64>>, NodeString ]), abs(seconds()-Time) } of
            ...
            { Mac, AbsDelta } ->
                error_logger:warning_msg ("expired DISCOVER (~p) from ~p:~p~n",
                                          [ AbsDelta,
                                            IP,
                                            InPortNo ]);
            _ ->
        ...
        error : { badmatch, _ } ->
          error_logger:warning_msg ("bad DISCOVER from ~p:~p~n", 
                                    [ list_to_binary (Rest),
                                      IP,
                                      InPortNo ])
    end,
